...
public class GetTagsTool extends AbstractTool {

    private static final VibeLogger.CategoryLogger LOG = VibeLogger.forClass(GetTagsTool.class);
    ...
    public GetTagsTool(EdtMetadataGateway gateway) {
        this.support = new EdtProjectAnalysisSupport(gateway);
    }

    @Override
    public String getDescription() {
        return "Возвращает список тегов EDT проекта и число объектов по каждому тегу."; //$NON-NLS-1$
    }
    ...
    protected CompletableFuture<ToolResult> doExecute(ToolParameters params) {
        return CompletableFuture.supplyAsync(() -> {
            String opId = LogSanitizer.newId("edt-get-tags"); //$NON-NLS-1$
            String projectName = ""; //$NON-NLS-1$
            LOG.info("[%s] START edt_get_tags", opId); //$NON-NLS-1$
            try {
                projectName = params.requireString("projectName").trim(); //$NON-NLS-1$
                JsonObject result = support.tags(projectName);
                ...
                LOG.error("[" + opId + "] edt_get_tags failed", e); //$NON-NLS-1$ //$NON-NLS-2$
                return failure(projectName, "GET_TAGS_FAILED", e.getMessage()); //$NON-NLS-1$
            }
        });
    }
