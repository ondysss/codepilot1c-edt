...
public final class ProviderNeutralToolSurfaceRewriteContributor implements ToolSurfaceContributor {
    ...
    private String overrideDescription(String toolName) {
        return switch (toolName) {
            ...
            case "write_file" -> "Overwrite existing workspace text files; may create project-root Code.md and documentation (*.md, *.txt). Line endings follow the target file, or the EDT preference for a new one, so mixed endings are not preserved. Never write .mdo/.form/.mxl/DCS artifacts directly; use semantic EDT tools."; //$NON-NLS-1$
